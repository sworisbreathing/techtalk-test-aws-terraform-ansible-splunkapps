...
class DeviceData:
    ...
    def __print_rpc_op_stats(self, op, sample_time, cur_time):
        ...
        print '[%s] op_type=%s, op_count=%d, ops_per_sec=%g, kB=%d, kBps=%g, kB_per_op=%g, retrans=%d, retrans_percent=%g, cumulative_rtt=%d, rtt_per_op=%g, cumulative_exe=%d, exe_per_op=%g' % (cur_time, op.lower(), ops, (ops / sample_time), kilobytes, (kilobytes / sample_time), kb_per_op, retrans, retrans_percent, rtt, rtt_per_op, exe, exe_per_op)
    ...
    def display_iostats(self, sample_time, which):
        ...
        print '[%s] total_op_count=%d, total_ops_per_sec=%g' % (cur_time, sends, (sends / sample_time))
        ...
        for o in ops:
            if self.__rpc_data.has_key(o):
                self.__print_rpc_op_stats(o, sample_time, cur_time)
...
def list_nfs_mounts(givenlist, mountstats):
    ...
    if len(givenlist) > 0:
        for device in givenlist:
            stats = DeviceData()
            # Allow <device> to be a subdirectory of a mountpoint.
            while True:
...
"""
    ...
    serverConf = splunk.bundle.getConf('server', sessionKey=session)

    pooling = (serverConf['pooling']['state'] == 'enabled')
    if pooling:
        # Search head pooling is enabled; filter down to pooling location.
        storage = serverConf['pooling']['storage']
        origdevices = [storage]

    inputConf = splunk.bundle.getConf('inputs', sessionKey=session)
    # This script never returns, so as a convenience, we've overloaded the
    # meaning of the "interval" attribute; it controls how often we print stats
    # to stdout.
    interval_seen = True
    interval = int(inputConf['script://./bin/nfs-iostat_sos.py']['interval'])
    ...
    if pooling:
        if len(devices) != 1:
            ...
            print '[%s] Printing stats for NFS mount: %s' % (time.asctime(), devices[0])
    else:
        if len(devices) == 0:
