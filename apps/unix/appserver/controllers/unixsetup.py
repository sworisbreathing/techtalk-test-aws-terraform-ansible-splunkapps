...
logger = logging.getLogger('splunk.unix_setup')

dir = os.path.join(util.get_apps_dir(), __file__.split('.')[-2], 'bin')
if not dir in sys.path:
    sys.path.append(dir)

from unix.models.input import MonitorInput, ScriptedInput
from unix.decorators import host_app
...
class UnixSetup(controllers.BaseController):
    ...
    def _redirect(self, host_app, app, endpoint):
        return self.make_url(['custom', host_app, 'unixsetup', app, endpoint])
