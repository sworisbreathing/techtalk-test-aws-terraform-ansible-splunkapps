Splunk.ModuleLoader= $.klass({
    ...
    onModuleLoadStatusChange: function(event, moduleId, statusInt) {
        if (!statusInt) {
            this.logger.error("Assertion failed - received null or invalid statusInt for module=", moduleId, " statusInt=", statusInt);
        }
        ...
        var extractFormValues = function(params) {
            ...
            for (var key in params) {
                if (key.indexOf(this.PERMALINK_ARGS_PREFIX) === 0) {
                    var tmp = key.substring(this.PERMALINK_ARGS_PREFIX.length);
                    if (formValues[tmp] != undefined && formValues[tmp] !== null) {
                        if ($.isArray(formValues[tmp]) === false) {
                            formValues[tmp] = new Array(formValues[tmp]);
                        }
    ...
    getLowestLoadState: function() {
        ...
        if (!this._constructedAllModules) {
            minimumStatus = Math.min(minimumStatus, Splunk.util.moduleLoadStates.WAITING_FOR_INITIALIZATION);
        } 
    ...
    _hasSuperclass: function(subclass, superclass) {
        ...
        while ((subclass.superclass) && i < 100) {
            ...
            if (subclass.superclass == superclass) {
                return true;
            }
