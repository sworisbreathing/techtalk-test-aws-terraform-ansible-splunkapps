Splunk.Module.sosNodes = $.klass(Splunk.Module.DispatchingModule, {
    ...
    DEFAULT_GRADIENT_LIST: [
        ...
        ['status_up_fill', '#F8FBEF', '#1a8907'],
        ['node_down_fill', '#F8FBEF', '#111111'],
    ...
    appendHosts: function(rack_obj, racks, downed, symbols) {
        ...
        for (k=0; k<racks.length; k++) {
            ...
            selectedoverlay=$('.ui-multiselect')[0];
 if (selectedoverlay['defaultValue']=="Memory") {
    $('#memorylegend').show();
}else{
    $('#memorylegend').hide();
}
 if (selectedoverlay['defaultValue']=="CPU") {
    $('#CPUlegend').show();
}else{
    $('#CPUlegend').hide();
}
                    ...
                    services = rack_obj[rack][selection];
                     ...
                     d3.select(this).append("rect")
                        ...
                        .attr("fill", function(s) {
                            if  (selectedoverlay['defaultValue']=="Status" ){
                                if (downed.hasOwnProperty(s)) {
                                    return "url(#node_down_fill)";
                                } else {
                                    return "url(#status_up_fill)";
                                }
                            }else{
 ...
 d3.select(this).append("svg:image")
                    ...
                    if (LabelArray.length>=18){
                       while (LabelArray.length>=18) {
                          LabelArray.pop(); LabelArray.pop(); LabelArray.pop();
                       }
                        d = LabelArray.join('') + "..."
                    }                         
...
return d; } )
                        .style("font-size", that.node_radius*.1+ 'px')
                        .attr("fill","black")
                        .attr("y", ypos-3)
                        .attr("x", xpos)
                        .attr("rx", 7)
    ...
    appendServices: function(s, services, xpos, ypos, symbols) {
        ...
        for (i=0; i < services.length; i++) { 
            ...
            // downed service decoration
             if (stat === 0  && service_down === false &&  selectedoverlay['defaultValue']=="Status" ) {
             //if (stat === 0 || stat === -1  && service_down === false &&  selectedoverlay['defaultValue']=="Status" ) {
                xpos += parseInt(pad*4.4, 10);
                ypos += parseInt(pad*4.4, 10);
                selection.append("svg:image")
                    .attr("xlink:href", Splunk.util.make_url("/static/app/sos/images/down_node_icon_35x35.png"))
    ...
    /*
     * find the appropriate fill given the metric and heatmap config 
     */
    getHeatmapFillStroke: function(metric, default_fill, default_stroke) {
       ...
       for (i=0; i < this.heatmap_config.rangemap.length ; i++) {
           ...
           this.appendGradientDef(["heatme"+i,'#F8FBEF',range.node_fill]);
           if (low <= met && high >= met) {
